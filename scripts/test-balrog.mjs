...
if (!apiKey) { console.error('No ANTHROPIC_API_KEY in .env'); process.exit(1); }
...
const STYLES = ['CHARACTER', 'MSV INFO', 'COMMUNITY QUESTION'];
...
Tone: casual discord speak. "boutta", "smt", "lowkey", "u" are fine. corny puns encouraged. just post like a normal person in a discord server. Output ONLY the message. Keep it short.`;
...
"what was ur first micro like?"`;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!raw) continue;
    text = raw.split('\n')[0].trim();
    if (!banned.test(text)) break;
    retries++;
  }
  if (banned.test(text)) text = 'who planning to be up regging they micro?';
  const extra = style === 1 ? ` (${char})` : (style === 2 ? ` (${topic})` : '');
  const retryNote = retries > 0 ? ` [${retries} retry${retries === 3 ? ' → fallback' : ''}]` : '';
  console.log(`${i}. [${STYLES[style - 1]}${extra}]${retryNote} ${text}`);
}
